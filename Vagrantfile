...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
  ...
  elsif Vagrant.has_plugin?('vagrant-hostmanager') then
    config.hostmanager.enabled = true
    config.hostmanager.manage_host = true
    config.hostmanager.aliases = settings['sites'].map { |site| site['map'] }
  elsif Vagrant.has_plugin?('vagrant-goodhosts') then
  ...
  config.vm.define "homestead_new" do |homestead_new|
    homestead_new.vm.box = "laravel/homestead"
  end
end
